...
public class PersistentMessageProducer {
    ...
    public PersistentMessageProducer(
                                     PersistentPool persistentPool, RocketMQTemplate rocketMQTemplate, PersistentProperties persistentProperties, SendCallback persistentMqSendCallback) {
    ...
    public <T extends IPersistent> SendResult syncSendMqNotice(T data) {
        ...
        return rocketMQTemplate.syncSend(persistentProperties.getTopic(), mqMessage);
    }

    public <T extends IPersistent> void asyncSendMqNotice(T data) {
        Message<PersistentMqNotice> mqMessage = createMqMessage(data);
        rocketMQTemplate.asyncSend(
                persistentProperties.getTopic(), mqMessage, persistentMqSendCallback);
    }
    ...
    public <T extends IPersistent> Message<PersistentMqNotice> createMqMessage(T data) {
        ...
        return MessageBuilder.withPayload(mqNotice).setHeader("KEYS", data.getId()).build();
    }
}
