...
public class MessageHandlerRegistrar
    implements SmartInitializingSingleton, ResourceLoaderAware, ApplicationContextAware {
  ...
  public void afterSingletonsInstantiated() {
    ...
    for (Class<?> handlerClazz : scanClasses(handlerPackages)) {
      ...
      for (Method method : handlerClazz.getDeclaredMethods()) {
        ...
        log.info(
            "Registering handler, msgId: {}, className : {}, Method: {}",
            msgId,
  ...
  private EnableMessageHandlerScan findScanAnno() {
    ...
    if (anno == null) {
      log.debug(
          "No @EnableMessageHandlerScan found on @SpringBootApplication class: {}",
          mainClass.getName());
      return null;
    }
    return anno;
