...
public class MessageHandlerRegistrar implements SmartInitializingSingleton, ResourceLoaderAware, ApplicationContextAware {
    ...
    public void afterSingletonsInstantiated() {
        ...
        for (Class<?> handlerClazz : scanClasses(handlerPackages)) {
            ...
            if (!applicationContext.containsBeanDefinition(handlerClazz.getName()) && applicationContext.getBeansOfType(handlerClazz).isEmpty()) {
                return;
            }
            ...
            for (Method method : handlerClazz.getDeclaredMethods()) {
                ...
                IContextHandle contextHandle = method.getReturnType() == void.class ? (ctx, req) -> {
                    mh.invoke(ctx, req);
                    return null;
                } : mh::invoke;
                ...
                log.info(
                        "Registering handler, msgId: {}, className : {}, Method: {}", msgId, handlerClazz.getName(), method.getName());
            }
        }
    }
    ...
    private EnableMessageHandlerScan findScanAnno() {
        ...
        if (anno == null) {
            log.debug(
                    "No @EnableMessageHandlerScan found on @SpringBootApplication class: {}", mainClass.getName());
            return null;
        }
        return anno;
    }
    ...
    private boolean isHandleMethod(Method method, Class<?> defaultMessageClazz) {
        ...
        if (!defaultMessageClazz.isAssignableFrom(method.getReturnType()) && method.getReturnType() != void.class) {
            return false;
        }
