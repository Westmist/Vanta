...
@RocketMQMessageListener(
        topic = "${persistent.topic:persistent-topic}", consumerGroup = "${rocketmq.consumer.group}", messageModel = MessageModel.CLUSTERING)
public class PersistentMessageConsumer implements RocketMQListener<PersistentMqNotice> {
    ...
    public PersistentMessageConsumer(
                                     PersistentPool persistentPool, RedisTemplate<String, IPersistent> redisTemplate, MongoTemplate mongoTemplate) {
